...
Kp = 10 * Mrb;  % Proportional gain
Kd = 5 * Mrb;   % Derivative gain
...
% Ensure initial theta is not near ±90°
etad_init(5) = 0.1;  % Small initial theta to avoid singularity
eta(:,1) = etad_init;
...
% Adjust initial velocity with a small ramp to avoid division by zero
ramp_init = 1e-6;  % Small initial ramp value
nu(:,1) = pinv(J) * (etad_dot_init / ramp_init);  % Initial velocity
...
for i = 1:length(t)-1
    ...
    tau(:,i) = J' * (M * etad_ddot(:,i) + Kd * e1dot + Kp * e1(:,i)) + tau_ff;  % PD only

    % Validate inputs before RK4
...
function [etad, etad_dot, etad_ddot] = generate_trajectory(t, trajectory_type, dt)
    ...
    % Smooth ramp for trajectory start (over ~2 seconds) with minimum value
    ramp = max(1e-6, 1 - exp(-t_vec / 2));
