...
% Control gains (tuned for smoother response)
Kp = 1 * Mrb; % Proportional gain
Kd = 2 * Mrb; % Derivative gain
% Ki = 0.1 * Mrb; % Integral gain (optional, uncomment if needed)
...
% Initialize with desired trajectory
...
% Define J2 (3x3 angular transformation matrix)
J2 = [1  sin(phi)*tan(theta)  cos(phi)*tan(theta);
      0  cos(phi)           -sin(phi);
      0  sin(phi)/cos(theta) cos(phi)/cos(theta)];

% Combine into full 6x6 Jacobian
J = [J1       zeros(3,3);
     zeros(3,3)  J2];

% Set initial velocity
nu(:,1) = pinv(J) * etad_dot_init;
%% Free flow velocity
Ux = 0.0; Uy = 0.0; Uz = 0;
...
% integral_error = zeros(6, length(t)); % Uncomment for integral control
...
trajectory_type = 'circle'; % Change to '8' or 'circle' to test others
...
for i = 1:length(t)-1
    ...
    % integral_error(:,i) = integral_error(:,i-1) + e1(:,i) * dt; % Uncomment for integral control
    tau(:,i) = J' * (M * etad_ddot(:,i) + Kd * e1dot + Kp * e1(:,i)); % + Ki * integral_error(:,i));
    ...
    f(:,i) = B \ tau(:,i);
    f(:,i) = max(min(f(:,i), 100), -100); % Increased force limit to ±100 N
    ...
    % Debug plots (uncomment to visualize early behavior)
    % if i < 100
    %     figure(2); subplot(2,1,1); plot(t(1:i), e1(1:3,1:i)'); title('Error'); legend('x', 'y', 'z'); grid on;
    %     subplot(2,1,2); plot(t(1:i), tau(1:3,1:i)'); title('Control Effort'); legend('x', 'y', 'z'); grid on;
    %     drawnow;
    % end
...
function [etad, etad_dot, etad_ddot] = generate_trajectory(t, trajectory_type, dt)
    ...
    % Smooth ramp for trajectory start (over ~2 seconds)
    ramp = 1 - exp(-t_vec / 2);
