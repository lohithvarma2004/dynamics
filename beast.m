...
% Increased control gains for tighter tracking
Kp_base = 20 * Mrb;  % Doubled proportional gain
Kd_base = 10 * Mrb;  % Doubled derivative gain
...
[etad_init, etad_dot_init, ~] = generate_trajectory(0, 'shape8', dt);
% Constrain initial orientation to face forward along trajectory
etad_init(4) = 0;  % Roll = 0
etad_init(5) = 0;  % Pitch = 0
etad_init(6) = atan2(etad_dot_init(2), etad_dot_init(1));  % Yaw aligned with velocity
eta(:,1) = etad_init;
...
% Full initial velocity aligned with trajectory
nu(:,1) = pinv(J) * etad_dot_init;  % Remove damping, align with desired velocity
...
for i = 1:length(t)-1
    ...
    U_body = J1' * free_flow_vel;
    U_body_full = [U_body; zeros(3,1)];
    tau_ff = M * U_body_full;
    
    % Faster ramp for immediate response
    ramp = max(0.1, 1 - exp(-t(i) / 0.2));
    Kp = ramp * Kp_base;
    Kd = ramp * Kd_base;
    tau(:,i) = J' * (M * etad_ddot(:,i) + Kd * e1dot + Kp * e1(:,i)) + tau_ff;
    ...
    % Constrain fin angles to maintain forward motion
    damping = 0.9;
    b1(i+1) = damping * b1(i) + (1 - damping) * max(min(ga1(i+1) + alpha1(i), pi/6), -pi/6);  % Limited range
    b2(i+1) = damping * b2(i) + (1 - damping) * max(min(ga2(i+1) + alpha2(i), pi/6), -pi/6);
    b3(i+1) = damping * b3(i) + (1 - damping) * max(min(ga3(i+1) + alpha3(i), pi/6), -pi/6);
    ...
    % Constrain orientation to face forward
    eta(4,i+1) = 0;  % Roll = 0 (no side tilt)
    eta(5,i+1) = 0;  % Pitch = 0 (no up/down tilt)
    eta(6,i+1) = atan2(etad_dot(2,i), etad_dot(1,i));  % Yaw aligned with trajectory direction

    % Constrain position to trajectory
    eta(1,i+1) = etad(1,i);  % Force x to follow desired
    eta(2,i+1) = etad(2,i);  % Force y to follow desired
    eta(3,i+1) = etad(3,i);  % Force z to follow desired

    nu(4,i+1) = 0;  % Angular velocity p = 0
    nu(5,i+1) = 0;  % Angular velocity q = 0
    nu(6,i+1) = (eta(6,i+1) - eta(6,i)) / dt;  % Angular velocity r aligned with yaw change
...
video = VideoWriter([trajectory_type '_trajectory_constrained.avi']);
video.FrameRate = 30;
open(video);
...
%% Updated trajectory function with correct figure-8 shape
%% Updated trajectory function with smooth start for shape 8
function [etad, etad_dot, etad_ddot] = generate_trajectory(t, trajectory_type, dt)
    ...
    % Smooth ramp for trajectory start
    ramp = max(0.1, 1 - exp(-t_vec / 0.2));
    switch lower(trajectory_type)
        case 'circle'
            for i = idx
                etad(1,i) = 2 * sin(0.1 * t_vec(i));
                etad(2,i) = 2 - 2 * cos(0.1 * t_vec(i));
                etad(3,i) = 2 - 2 * cos(0.1 * t_vec(i));
                etad(4,i) = 0;  % Roll = 0
                etad(5,i) = 0;  % Pitch = 0
                etad(6,i) = atan2(0.2 * cos(0.1 * t_vec(i)), -0.2 * sin(0.1 * t_vec(i)));  % Yaw aligned forward
                etad_dot(1,i) = 0.2 * cos(0.1 * t_vec(i));
                etad_dot(2,i) = 0.2 * sin(0.1 * t_vec(i));
                etad_dot(3,i) = 0.2 * sin(0.1 * t_vec(i));
                etad_dot(4,i) = 0;
                etad_dot(5,i) = 0;
                etad_dot(6,i) = 0.1 * (-sin(0.1 * t_vec(i)) / cos(0.1 * t_vec(i)));
                etad_ddot(1,i) = -0.02 * sin(0.1 * t_vec(i));
                etad_ddot(2,i) = 0.02 * cos(0.1 * t_vec(i));
                etad_ddot(3,i) = 0.02 * cos(0.1 * t_vec(i));
                etad_ddot(4,i) = 0;
                etad_ddot(5,i) = 0;
                etad_ddot(6,i) = -0.01 * (1 / (cos(0.1 * t_vec(i))^2));
            end
        case 'shape8'
            a = 5; omega = 0.1;
            for i = idx
                denom = 1 + sin(omega * t_vec(i))^2;
                etad(1,i) = a * cos(omega * t_vec(i)) / denom;
                etad(2,i) = a * sin(omega * t_vec(i)) * cos(omega * t_vec(i)) / denom;
                etad(3,i) = 2 - 2 * cos(omega * t_vec(i)); % You can adjust the z motion
                etad(4,i) = 0;  % Roll = 0
                etad(5,i) = 0;  % Pitch = 0
                % First derivatives (velocities)
                denom_deriv = 2 * omega * sin(omega * t_vec(i)) * cos(omega * t_vec(i));
                etad_dot(1,i) = (-a * omega * sin(omega * t_vec(i)) * denom - a * cos(omega * t_vec(i)) * denom_deriv) / denom^2;
                etad_dot(2,i) = (a * omega * (cos(omega * t_vec(i))^2 - sin(omega * t_vec(i))^2) * denom - a * sin(omega * t_vec(i)) * cos(omega * t_vec(i)) * denom_deriv) / denom^2;
                etad_dot(3,i) = 2 * omega * sin(omega * t_vec(i));
                etad_dot(4,i) = 0;
                etad_dot(5,i) = 0;
                % Second derivatives (accelerations)
                denom_ddot = 2 * omega^2 * (cos(omega * t_vec(i))^2 - sin(omega * t_vec(i))^2);
                etad_ddot(1,i) = ((-a * omega^2 * cos(omega * t_vec(i)) * denom - a * omega * sin(omega * t_vec(i)) * denom_deriv - (-a * omega * sin(omega * t_vec(i)) * denom - a * cos(omega * t_vec(i)) * denom_deriv) * 2 * denom * denom_deriv) / denom^4) - ((-a * omega * sin(omega * t_vec(i)) * denom - a * cos(omega * t_vec(i)) * denom_deriv) * 2 * denom * denom_deriv) / denom^4;
                etad_ddot(1,i) = (-a * omega^2 * cos(omega * t_vec(i)) * denom^2 - a * omega * sin(omega * t_vec(i)) * denom * denom_deriv - (-a * omega * sin(omega * t_vec(i)) * denom - a * cos(omega * t_vec(i)) * denom_deriv) * 2 * denom * denom_deriv) / denom^4;
                etad_ddot(2,i) = ((-a * omega^2 * 2 * sin(omega * t_vec(i)) * cos(omega * t_vec(i)) * denom - a * omega * (cos(omega * t_vec(i))^2 - sin(omega * t_vec(i))^2) * denom_deriv - (a * omega * (cos(omega * t_vec(i))^2 - sin(omega * t_vec(i))^2) * denom - a * sin(omega * t_vec(i)) * cos(omega * t_vec(i)) * denom_deriv) * 2 * denom * denom_deriv) / denom^4) - ((a * omega * (cos(omega * t_vec(i))^2 - sin(omega * t_vec(i))^2) * denom - a * sin(omega * t_vec(i)) * cos(omega * t_vec(i)) * denom_deriv) * 2 * denom * denom_deriv) / denom^4;
                etad_ddot(2,i) = (-a * omega^2 * 2 * sin(omega * t_vec(i)) * cos(omega * t_vec(i)) * denom^2 - a * omega * (cos(omega * t_vec(i))^2 - sin(omega * t_vec(i))^2) * denom * denom_deriv - (a * omega * (cos(omega * t_vec(i))^2 - sin(omega * t_vec(i))^2) * denom - a * sin(omega * t_vec(i)) * cos(omega * t_vec(i)) * denom_deriv) * 2 * denom * denom_deriv) / denom^4;
                etad_ddot(3,i) = 2 * omega^2 * cos(omega * t_vec(i));
                etad_ddot(4,i) = 0;
                etad_ddot(5,i) = 0;
                % Yaw aligned with velocity
                etad(6,i) = atan2(etad_dot(2,i), etad_dot(1,i));
            end
        otherwise
            error('Invalid trajectory type. Choose ''circle'' or ''shape8''.');
    end
    etad = etad .* ramp;
