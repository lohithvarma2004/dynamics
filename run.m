...
Kp_base = 0 * Mrb;  % Doubled proportional gain
Kd_base = 0 * Mrb;  % Doubled derivative gain
...
[etad_init, etad_dot_init, ~] = generate_trajectory(0, 'm', dt);
...
function [etad, etad_dot, etad_ddot] = generate_trajectory(t, trajectory_type, dt)
    ...
    switch lower(trajectory_type)
        ...
        otherwise
            error('Invalid trajectory type. Choose ''circle'', ''shape8'', or ''m''.');
    end
    etad = etad .* ramp;
